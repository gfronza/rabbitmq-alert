...
class RabbitMQAlert:
    ...
    def check_queue_conditions(self, options):
        ...
        if ready_size and messages_ready > ready_size:
            self.send_notification(options, "%s: messages_ready > %s" % (options.queue, str(ready_size)))

        if unack_size and messages_unacknowledged > unack_size:
            self.send_notification(options, "%s: messages_unacknowledged > %s" % (options.queue, str(unack_size)))

        if total_size and messages > total_size:
            self.send_notification(options, "%s: messages > %s" % (options.queue, str(total_size)))
...
if __name__ ==  "__main__":
    ...
    options = rabbitmqalert.setup_options()
